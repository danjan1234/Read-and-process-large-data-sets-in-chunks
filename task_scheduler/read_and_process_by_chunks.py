...
from task_scheduler.reader import Reader
from task_scheduler.log import Log
...
class ReadProcessByChunks(object):
    ...
    def __init__(self, file_path, save_dir_path, purge_save_directory=False, save_mode=0, delayed_save=False,
                 ...
                 n_buffered_results=-1, func=None, func_kwargs={}, n_jobs=-1, chunk_limit=-1, dist_interval=0.1,
                 sum_prefix='sum'):
        """
        Arguments:
            ...
            chunk_limit: uint, default -1
                ...
                debugging purpose
            dist_interval: float, default=0
                The time interval when distributing the jobs. This parameter is used to avoid rush read or right
        ...
        self._dist_interval = dist_interval
        self._sum_prefix = sum_prefix

        # Create the log object
        if isinstance(self._file_path, pd.DataFrame):
            log_dir_path = os.path.split(self._save_dir_path)[0]
        else:
            log_dir_path = self._save_dir_path
        self._log = Log(os.path.join(log_dir_path, 'log.txt'), log_interval=10)
    ...
    def _read_chunks_to_queue(self):
        ...
        try:
            ...
            while True:
                ...
                chunk_id += 1
                time.sleep(self._dist_interval)
        ...
        while just_started or n_proc_alive > 0 or self._result_queue.qsize() > 0:
            ...
            if self._result_queue.qsize() > 0:
                ...
                for summary_id, data in enumerate(queue_data):
                    ...
                    # Find out the save path
                    if file_path is None:
                        file_save_path = self._save_dir_path
                    else:
                        file_save_path = os.path.join(self._save_dir_path, file_path)

                    # Further refine save path based on the save mode
                    hdf_kwargs = {}
                    ...
                    # Delay save or save immediately?
...
if __name__ == '__main__':
    ...
    print("\nRead multiple files (each file is read as a whole):")
    ...
    print("\nRead multiple files (each file is split to chunks of fixed sizes):")
    file_path = r'..\test_data\test_data_set\[^.]*fixed.h5'
    save_dir_path = r'..\test_data\test_data_set_summary'
    shutil.rmtree(save_dir_path)
    s = ReadProcessByChunks(file_path, save_dir_path, chunk_size=3, func=count_rows, hdf_format='table')
    ...
    print("\nRead multiple file (each file is split to chunks of varying size based on a certain column):")
    file_path = r'..\test_data\test_data_set\[^.]*fixed.h5'
    save_dir_path = r'..\test_data\test_data_set_summary'
    shutil.rmtree(save_dir_path)
    ...
    print("\nRead multiple files (w/ grouping, each file is split to chunks of varying size):")
    file_path = r'..\test_data\test_data_set\[^.]*fixed.h5'
    save_dir_path = r'..\test_data\test_data_set_summary'
    shutil.rmtree(save_dir_path)
